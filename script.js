...
const translations = {
    ar: {
        hero_title: 'اكتشف عالم <br><span class="gradient-text">الاشتراكات الرقمية</span>',
        ...
        sold_out: 'نفذت الكمية' // بدون إيموجي لتطابق الصورة 2 تماماً
    },
    fr: {
        hero_title: 'Découvrez le monde des <br><span class="gradient-text">Abonnements</span>',
...
function renderProducts(productsList) {
    ...
    for (const [category, items] of Object.entries(groupedProducts)) {
        ...
        items.forEach(p => {
            ...
            // القائمة المنسدلة الآن تعرض "المدة فقط"
            ...
            // إضافة السعر في الأسفل خارج القائمة المنسدلة
            card.innerHTML = `
                <div style="width:100%;">
                    ${imgHTML}
                    <h3 style="font-size:0.95rem; margin-bottom:5px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;">${p.name}</h3>
                    ${selectHTML}
                </div>
                <div>
...
function syncDisplay(productId) {
    ...
    // تحديث السعر المعروض في الأسفل بناءً على القائمة المنسدلة
