...
function renderProducts(productsList) {
    ...
    for (const [category, items] of Object.entries(groupedProducts)) {
        ...
        items.forEach(p => {
            ...
            // ترتيب الأزرار ليتوافق مع CSS و LTR (الزائد يسار، الناقص يمين)
            card.innerHTML = `
                ...
                <div class="qty-controls">
                    <button class="btn-qty plus" onclick="updateCart('${p.id}', 1)">+</button>
...
function updateCartUI() {
    ...
    if (totalItems > 0) floatBar.style.display = 'flex';
    else { floatBar.style.display = 'none'; cartItemsDiv.innerHTML = '<p style="text-align:center; color:var(--text-sub);">السلة فارغة</p>'; }
...
function calcTotal(baseTotal = null) {
    if (baseTotal === null) {
        baseTotal = 0;
        for (const id in cart) baseTotal += (parseInt(cart[id].price) * cart[id].qty);
    }
...
// 🚀 نظام الإرسال المزدوج (واتساب + سيرفر)
async function checkout() {
    ...
    try {
        ...
        for (const id in cart) {
            let line = `- ${cart[id].name} (${cart[id].duration}) | الكمية: ${cart[id].qty}\n`;
            orderDetails += line;
            waMessage += line;
        }
        ...
        // 1. الإرسال الصامت لجوجل (التليجرام)
        fetch(APPS_SCRIPT_URL, {
            method: 'POST',
            headers: { 'Content-Type': 'text/plain;charset=utf-8' },
            body: JSON.stringify(payload) 
        }).catch(e => console.log("Apps script error but continuing..."));

        // 2. تحويل الزبون فوراً للواتساب
        alert("✅ تم تجهيز طلبك! سيتم تحويلك للواتساب الآن لإرسال الوصل.");
        ...
        btn.disabled = false;
        btn.innerHTML = '<i class="fab fa-whatsapp" style="font-size:1.3rem;"></i> <span data-i18n="checkout_btn">تأكيد وإرسال عبر واتساب</span>';
    }
}
