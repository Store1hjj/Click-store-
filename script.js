...
function checkout() {
    ...
    const payload = { phone: phone, items: itemsText, total: total, method: method, promo: promo };
    ...
    const send = () => {
        ...
        .then(() => {
            ...
            const waText = `*طلب جديد من المتجر* 🛒%0A%0A*رقم الهاتف:* ${phone}%0A%0A*المنتجات:*%0A${cart.map(i => `▪️ ${i.name} (${i.duration}) ✖️ ${i.qty}`).join('%0A')}%0A%0A*طريقة الدفع:* ${method}%0A*الإجمالي:* ${total}%0A%0A(✅ تم رفع صورة الوصل بنجاح في قاعدة البيانات)`;
            ...
            document.getElementById('cartContentArea').innerHTML = `
                <div style="text-align:center; padding: 40px 20px;">
                    ...
                    <p style="color: var(--text-sub); margin-bottom: 20px;">${currentLang==='ar'?'تم إرسال وصل الدفع والتفاصيل إلى الإدارة.':'Le reçu et les détails ont été envoyés.'}</p>
                    <button onclick="window.location.reload()" style="padding: 15px 30px; background: var(--primary); color: white; border: none; border-radius: 12px; font-weight: bold; font-size:1.1rem; cursor: pointer;">${currentLang==='ar'?'العودة للمتجر':'Retour à la boutique'}</button>
                </div>`; 
            cart = [];
    ...
    reader.onload = e => { 
        ...
        payload.filename = `rec_${phone}.jpg`; 
        send(); 
    };
