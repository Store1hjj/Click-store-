...
const translations = {
    ar: {
        ...
        sold_out: 'نفذت الكمية ❌' // تمت إضافة ترجمة الختم
    },
    fr: {
...
function renderProducts(productsList) {
    ...
    productsList.forEach(product => {
        // تم إزالة السطر الذي يخفي المنتج نهائياً (if product.active === false return;)
    ...
    for (const [category, items] of Object.entries(groupedProducts)) {
        ...
        items.forEach(p => {
            ...
            // التحقق هل المنتج متوفر أم نفذت كميته
            const isActive = p.active !== false; 

            // تعطيل قائمة الأسعار إذا نفذت الكمية
            ...
            // إضافة ختم نفاذ الكمية وجعل الصورة باهتة
            let imgHTML = `
...
function modifyCart(productId, change) {
    ...
    if (!product || product.active === false) return; // منع إضافة منتج نافذ
    ...
    for (const key in cart) {
        ...
        cartItemsDiv.innerHTML += `
            <div style="display:flex; justify-content:space-between; margin-bottom:10px; border-bottom:1px solid rgba(255,255,255,0.1); padding-bottom:5px;">
                <span style="direction:ltr; text-align:left;">${item.qty} x ${item.duration} ${item.name}</span>
                <span style="color:var(--accent); direction:ltr;">${item.price * item.qty} ${subText}</span>
            </div>
        `;
    }
