...
const APPS_SCRIPT_URL = 'https://script.google.com/macros/s/AKfycbwSUimBtNTIaL8CfeR5tC2GRs5-4jdgLD5HLxdHXxNkx7erDlqTAt57-WSa8t3kQzfZ1g/exec';
...
function toggleLanguage() {
    ...
    document.querySelectorAll('[data-i18n]').forEach(el => {
        ...
        if (translations[currentLang][key]) {
            if (key === 'checkout_btn') {
                el.innerHTML = `<i class="fab fa-whatsapp" style="font-size:1.3rem; margin-inline-end: 8px;"></i>${translations[currentLang][key]}`;
            } else if (key === 'cart_title') {
                el.innerHTML = `<i class="fas fa-shopping-cart" style="margin-inline-end: 8px;"></i>${translations[currentLang][key]}`;
            } else {
...
async function checkout() {
    ...
    try {
        ...
        // 🚀 هنا نظام ضغط الصورة لكي يتم الإرسال في جزء من الثانية
        const base64File = await getBase64(file);
        
        let safeBase64 = base64File;
        if (base64File.includes(',')) {
            safeBase64 = base64File.split(',')[1];
        }
        ...
        const payload = {
            ...
            image: safeBase64, 
            filename: "receipt_" + Date.now() + ".jpg" // اسم آمن للصورة
        };

        // ⏳ إرسال فائق السرعة
        const response = await fetch(APPS_SCRIPT_URL, {
            method: 'POST',
            body: JSON.stringify(payload) // نزعنا الهيدر لتفادي أي حظر من المتصفح (CORS)
        });
        
        let fileUrl = await response.text();
        fileUrl = fileUrl.trim(); // تنظيف الرابط من أي مسافات

        // 🛡️ التأكد أن الرابط سليم وأنه يطبع في الواتساب
        if (fileUrl && fileUrl.startsWith("http")) {
            waMessage += `\n\n*🔗 رابط وصل الدفع:*\n${fileUrl}`;
        } else {
            waMessage += `\n\n*🔗 رابط وصل الدفع:*\n(تم إرسال الصورة للأدمن بنجاح)`;
        }
        ...
        alert(currentLang === 'ar' ? "❌ حدث خطأ في الشبكة، يرجى المحاولة مرة أخرى." : "❌ Erreur réseau, veuillez réessayer.");
    } finally {
        btn.disabled = false;
        btn.innerHTML = `<i class="fab fa-whatsapp" style="font-size:1.3rem; margin-inline-end: 8px;"></i>${translations[currentLang]['checkout_btn']}`;
    } 
}

// 🚀 نظام ضغط الصور الفائق السرعة (يضغط الصورة لـ 100kb قبل إرسالها)
    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = (event) => {
            ...
            img.onload = () => {
                ...
                // ضغط الصورة إلى جودة 60% لضمان سرعة الإرسال
