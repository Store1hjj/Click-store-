...
async function checkout() {
    ...
    try {
        ...
        // ضغط الصورة لسرعة الإرسال
        const base64File = await getBase64(file);
        let safeBase64 = base64File.includes(',') ? base64File.split(',')[1] : base64File;
        ...
        const payload = {
            ...
            image: safeBase64, 
            filename: "receipt_" + phone + ".jpg" 
        };

        // 🚀 الإرسال الصامت في الخلفية لتفادي أخطاء CORS وتأخير الزبون
        fetch(APPS_SCRIPT_URL, {
            method: 'POST',
            body: JSON.stringify(payload),
            mode: 'no-cors' // هذا السطر السحري يمنع المتصفح من حظر العملية
        }).catch(e => console.log("Sent in background"));

        // إضافة رسالة التأكيد للواتساب بدل الرابط الذي تحظره جوجل
        waMessage += `\n\n*🧾 وصل الدفع:* تم رفعه وإرساله للإدارة بنجاح ✅`;

        // التحويل المباشر والفوري للواتساب بدون انتظار
        ...
        alert(currentLang === 'ar' ? "❌ حدث خطأ، يرجى المحاولة مرة أخرى." : "❌ Erreur, veuillez réessayer.");
    } finally {
        btn.disabled = false;
