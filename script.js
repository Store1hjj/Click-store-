...
function calcTotal() {
    ...
    const fileUploadSection = document.querySelector('.file-upload-wrapper'); // استدعاء زر الرفع
    ...
    fileUploadSection.style.display = 'block'; // إظهار زر رفع الصورة كحالة افتراضية
    ...
    if(method === 'flexy') { 
        ...
        fileUploadSection.style.display = 'none'; // 👈 إخفاء زر رفع الصورة إذا اختار فليكسي
    } 
...
function checkout() {
    ...
    // إجبارية رفع الصورة (إلا إذا كان فليكسي)
    ...
    // التحقق: إذا فليكسي، نرسل بدون صورة. وإذا طرق أخرى، نقرأ الصورة ونرسلها.
    if(method === 'flexy') {
        sendToServer("", ""); // لا توجد صورة
    } else {
