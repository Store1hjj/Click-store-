...
// ✅ تم ربط السكربت بالرابط الجديد الخاص بك
...
function toggleLanguage() {
    ...
    document.querySelectorAll('[data-i18n]').forEach(el => {
        ...
        if (translations[currentLang][key]) {
            // إضافة الأيقونة ديناميكياً لتفادي مشكلة الأيقونة المزدوجة
            if (key === 'checkout_btn') {
...
async function checkout() {
    ...
    try {
        ...
        // 🛡️ حماية تشفير الصورة (لضمان قراءتها دائماً وعدم فقدان البيانات)
        ...
        const payload = {
            ...
            image: safeBase64, // إرسال الصورة المحمية
            filename: file.name
        };

        // ⏳ انتظار رد جوجل سكريبت
        ...
        // دمج رابط الصورة إذا تم إرجاعه بنجاح
        if (fileUrl && fileUrl !== "Error" && fileUrl !== "No Data" && fileUrl !== "بدون صورة") {
            waMessage += `\n\n*🔗 رابط وصل الدفع:*\n${fileUrl}`;
        }
        ...
        // التحويل الإجباري في نفس النافذة
