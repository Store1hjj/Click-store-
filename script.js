...
function checkout() {
    ...
    reader.onload = e => { 
        ...
        const payload = { 
            ...
            promo: promo,
            image: base64Data, 
            filename: `rec_${phone}.jpg` 
        };

        // حذفنا no-cors باش نقدروا نقراو الرابط اللي يرجعو درايف
        fetch(API_URL, { 
            method: 'POST', 
            headers: { 'Content-Type': 'text/plain;charset=utf-8' }, 
            body: JSON.stringify(payload) 
        })
        .then(response => response.text()) // هنا نستقبل رابط جوجل درايف
        .then(driveLink => {
            localStorage.setItem('admin_orders_count', (parseInt(localStorage.getItem('admin_orders_count')) || 0) + 1);
            
            // تجهيز نص الواتساب وتضمين رابط درايف مباشرة
            let finalLink = (driveLink && driveLink.startsWith('http')) ? driveLink : '(تم الرفع إلى قاعدة البيانات)';
            
            const waText = `*طلب جديد من المتجر* 🛒\n\n*رقم الهاتف:* ${phone}\n\n*المنتجات:*\n${cart.map(i => `▪️ ${i.name} (${i.duration}) ✖️ ${i.qty}`).join('\n')}\n\n*طريقة الدفع:* ${method}\n*الإجمالي:* ${total}\n\n*رابط وصل الدفع:* 👇\n${finalLink}`;
            ...
            // التوجيه للواتساب باستخدام location.href (هذا يمنع المتصفح من حظره)
            setTimeout(() => {
                window.location.href = `https://wa.me/${ADMIN_PHONE}?text=${encodeURIComponent(waText)}`;
            }, 1000);

        }).catch((err) => { 
            // في حال فشل الاتصال، نقوم بإرسال الطلب للواتساب احتياطياً بدون تعليق الزبون
            const fallbackText = `*طلب جديد من المتجر* 🛒\n\n*رقم الهاتف:* ${phone}\n*الإجمالي:* ${total}\n(ملاحظة: راجع قاعدة البيانات للتفاصيل والوصل)`;
