...
function checkout() {
    ...
    reader.onload = e => { 
        ...
        // إرسال البيانات (بدون no-cors لكي نستقبل الرابط من السيرفر)
        fetch(API_URL, { 
            method: 'POST', 
        ...
        .then(responseText => {
            ...
            // استخراج رابط الصورة الحقيقي من استجابة Google Script
            let finalLink = "لم يتم إرجاع الرابط من السيرفر (Google Script)";
            let urlMatch = responseText.match(/https?:\/\/[^\s"']+/); // يبحث عن أي رابط داخل النص
            if(urlMatch) {
                finalLink = urlMatch[0];
            } else if (responseText.includes("http")) {
                finalLink = responseText;
            }
            ...
            const fallbackText = `*طلب جديد من المتجر* 🛒\n\n*رقم الهاتف:* ${phone}\n*الإجمالي:* ${total}\n(ملاحظة: راجع التليجرام لرؤية وصل الدفع)`;
